...
#define AXIDMA_VFIO_GROUP 	"/dev/vfio/0"			// dmesg | grep group, look for axi dma address
#define AXIDMA_DEVICE		"a0000000.dma"			// the address for the device that is with the group

#define DMA_TRANSFER_SIZE_4KPAGES	(1)			// 4K pages
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "cr before reset 0x%x\n", *(u32*)base_regs);
	fprintf(stderr, "sr before reset 0x%x\n", *(u32*)(base_regs+0x04));
	...
	*(u32 *)(base_regs + 0x30) = 4;
	...
	//*(u32 *)(base_regs + 0x00) |= 0x1;
	//*(u32 *)(base_regs + 0x30) |= 0x1;
	fprintf(stderr, "cr after reset 0x%x\n", *(u32*)base_regs);
	fprintf(stderr, "sr after reset 0x%x\n", *(u32*)(base_regs+0x04));
	...
	*(u32 *)(base_regs + 0x58) = size_to_map;
	...
	fprintf(stderr, "cr after start rx 0x%x\n", *(u32*)base_regs);
	fprintf(stderr, "sr after start rx 0x%x\n", *(u32*)(base_regs+0x04));
	...
	*(u32 *)(base_regs + 0x28) = size_to_map;
	...
	fprintf(stderr, "cr after start tx 0x%x\n", *(u32*)base_regs);
	fprintf(stderr, "sr after start tx 0x%x\n", *(u32*)(base_regs+0x04));
	...
	for(c = 0; c < tot; c++) {
		if(src_ptr[c] != dst_ptr[c]) {
			printf("test failed! - %d - 0x%lx - 0x%lx\n", c, src_ptr[c], dst_ptr[c]);
			fail_count++;
		}
	}
	if (!fail_count)
