...
case "$1" in
  start)
    ...
    start-stop-daemon --start --quiet --background --make-pidfile --pidfile /var/run/$APSETUPUTIL_PID_NAME.pid --exec $APSETUPUTIL
    echo "."
    ;;
  stop)
    echo -n "Stopping Ultra96 AP setup daemon"
    start-stop-daemon --stop --quiet --pidfile /var/run/$APSETUPUTIL_PID_NAME.pid
    ;;
  *)
